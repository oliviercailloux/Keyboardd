...
* `KeyboardMap`: x key names to keysym entries; typically defined in a set of local (or embedded) “symbol” configuration files; can be obtained using `XkbSymbolsReader`.

.Mapping
[[Mapping]]
image::Mapping.svg[Mapping, opts=inline]

* `Mnemonics`: a set of keysym mnemonics, together with information per mnemonic (the keysym code it maps to, whether it is an alias, whether it is deprecated, which UCP corresponds to it if any, …); can be obtained from a standard (embedded) `xkbcommon-keysyms.h` file.
...
* `XKeyNamesRepresenter`: x key names to representations; may be obtained by combining a `CanonicalKeyboardMap` (itself associating a `KeyboardMap` to mnemonics) and your own representations (as a map from `CanonicalKeysymEntry` to `Representation`)
...
(In practice, this seems unlikely to me.)
