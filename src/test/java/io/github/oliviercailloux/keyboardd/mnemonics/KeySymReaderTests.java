...
public class KeySymReaderTests {
  ...
  public void testReadDeprecatedAlias() throws Exception {
    ...
    ImmutableSet<ParsedMnemonic> mns = KeySymReader.parse(source);
    assertEquals(ImmutableSet.of(ParsedMnemonic.deprecatedAlias("KP_Page_Up",
        Integer.parseInt("ff9a", 16), "deprecated alias for KP_Prior", "KP_Prior")), mns);
  }
  ...
  public void testReadAlias() throws Exception {
    ...
    assertEquals(ImmutableSet.of(ParsedMnemonic.alias("ISO_Group_Shift",
        Integer.parseInt("ff7e", 16), "Alias for Mode_switch", "Mode_switch")), mns);
  }
  ...
  public void testReadDeprecatedComment() throws Exception {
    ...
    assertEquals(ImmutableSet.of(ParsedMnemonic.deprecatedComment("dead_small_schwa",
        Integer.parseInt("fe8a", 16), "deprecated, remove in 2025", "remove in 2025")), mns);
  }
  ...
  public void testReadDeprecatedCommentMissp() throws Exception {
    ...
    assertEquals(ImmutableSet.of(ParsedMnemonic.deprecatedComment("guillemotleft",
        Integer.parseInt("ab", 16), "deprecated misspelling", "misspelling")), mns);
  }
  ...
  public void testReadDeprecated() throws Exception {
    ...
    ImmutableSet<ParsedMnemonic> mns = KeySymReader.parse(source);
    assertEquals(
        ImmutableSet.of(ParsedMnemonic.deprecated("quoteleft", Integer.parseInt("60", 16))), mns);
  }
  ...
  public void testLatest() throws Exception {
    ImmutableSet<ParsedMnemonic> mns = KeySymReader.latestRaw();
    ...
    { /**
       * grep "^#define" "xkbcommon-keysyms - 238d13.h" | grep -c "/\* [aA]lias for "; grep
       * "^#define" "xkbcommon-keysyms - 238d13.h" | grep -c "/\* Same as "
       */
      ImmutableSet<ParsedMnemonic> matching = mns.stream().filter(s -> s.alias() && !s.deprecated())
          .collect(ImmutableSet.toImmutableSet());
      assertEquals(13 + 9, matching.size());
    }
    ...
    { /**
       * grep "^#define" "xkbcommon-keysyms - 238d13.h" | grep "/\* deprecated" | grep -c -v "/\*
       * deprecated [aA]lias for "
       */
      ImmutableSet<ParsedMnemonic> matching =
          mns.stream().filter(s -> !s.comment().isEmpty() && s.deprecated() && !s.alias())
              .collect(ImmutableSet.toImmutableSet());
      assertEquals(44, matching.size());
    }
    { /** grep "^#define" "xkbcommon-keysyms - 238d13.h" | grep -c "/\* deprecated [aA]lias for " */
      ImmutableSet<ParsedMnemonic> matching = mns.stream().filter(s -> s.alias() && s.deprecated())
          .collect(ImmutableSet.toImmutableSet());
      assertEquals(59, matching.size());
    }
